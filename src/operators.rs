...
pub fn rms_norm(y: &mut Tensor<f32>, x: &Tensor<f32>, w: &Tensor<f32>, epsilon: f32) {
    ...
    let y_size = y.size(); //y的长度
    let y_ = unsafe { y.data_mut() }; //y的数据
    for y_i_j in 0..y_size {
        let mut rms = 0 as f32;
        for x_i_j in y_i_j / w.size() * w.size()..(y_i_j / w.size() * w.size() + w.size()) {
            rms += x.data()[x_i_j].powi(2);
        }
        rms = (rms / w.size() as f32 + epsilon).sqrt(); //均方根计算完毕
        y_[y_i_j] = w.data()[y_i_j % w.size()] * x.data()[y_i_j] / rms;
    }
}
